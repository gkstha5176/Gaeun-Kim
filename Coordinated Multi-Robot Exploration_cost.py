...
while not gameExit:
    ...
    for x in range(1, len(grid.squares) - 1):
        for y in range(1, len(grid.squares[0]) - 1):
            if grid.squares[x][y].get_type() == "empty":
                grid.squares[x][y].value = 1000000
            elif grid.squares[x][y].get_type() == "bot":
    ...
    while k < 5:
        for x in range(1, len(grid.squares) - 1):
            for y in range(1, len(grid.squares[0]) - 1):
                if grid.squares[x][y].get_value() == None:
                    continue
                grid.squares[x][y].minvalue = grid.squares[x][y].get_value()
                for i in [-1, 0, 1]:
                    for j in [-1, 0, 1]:
                        if grid.squares[x + i][y + j].get_value() == None:
                            grid.squares[x + i][y + j].value = grid.squares[x][y].get_value()
                        if grid.squares[x + i][y + j].get_value() + (i**2 + j**2)**(0.5) * 0.1 < grid.squares[x][y].get_minvalue() :
                            grid.squares[x][y].minvalue = grid.squares[x + i][y + j].get_value() + (i**2 + j**2)**(0.5) * 0.1
                grid.squares[x][y].value = grid.squares[x][y].get_minvalue()
        k += 1
    ...
    if startSearching:
        ...
        if len(grid.get_frontier_cells()) == 0:
            startSearching = False
        ...
        while len(botsWithoutTargetPoints) > 0:
            ...
            for botPosition in botsWithoutTargetPoints:
                ...
                for i in range(len(tempV)):
                    ...
                    Coor_x = vCoordinates[0]
                    Coor_y = vCoordinates[1]
                    vValue = tempV[i]["distance"]
                    
                    
                    U = grid.frontierCellsDict[vCoordinates]
                    cost = grid.squares[Coor_x][Coor_y].minvalue

                    
                    utility = U - (0.01 * vValue) - cost * 0.01
